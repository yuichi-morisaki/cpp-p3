...
double define_var(string n, double v, bool is_constant)
{
    if (is_declared(n))
        error(n, " declared twice");

    var_table.push_back(Variable(n, v, is_constant));
...
double declaration(bool is_constant)
{
    ...
    return define_var(name, val, is_constant);
}
...
try
{
    ...
    define_var("pi", 3.1415926535, true);
    define_var("e", 2.7182818284, true);
